...
class KrishaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        self.page += 1
        with open('krisha.csv', 'w', encoding='utf-8') as f:
            ...
            for i in realtors:
                 ...
                 spec = i.css('div.pr_block').css('div.speciality::text').extract_first()
                 numbers = i.css('div.pr_block').css('div.phones::text').extract_first().replace(' ', '').replace('(', '').replace(')', '').split(',')
                # photo = i.css('div.image_cover').extract_first()
                # name = i.css('div.pr_block').css('a.name::text').extract_first()
                # numbers = i.css('div.pr_block').extract_first().replace(' ', '').replace('(', '').replace(')', '').split(',')
                 f.write(u'{},{}\n,{}'.format(name, spec, numbers))
